 ...
 $member-> set('id',$_GET['id']);
        if(isset($_POST['btnUpdate'])){
          ...
          if(isset($_POST['status'])&&!empty($_POST['status'])){
            ...
            $member->set('Updated_by',$_SESSION['admin_username']);
            $member->set('Updated_date',date('Y-m-d H:i:s'));
            $status=$member->edit();
}
    ...
    <tr>
     ...
     <tr>
      ...
      <td><input type="File" name="image" required="">
        <br>
        <img src="image/member/<?php echo $data[0]->image?>" width="50px">
        <?php
        if(isset($err['image'])){
          echo $err['image'];
        }
